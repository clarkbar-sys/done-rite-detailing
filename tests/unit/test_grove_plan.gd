...
func _row(count: int) -> Array[Transform3D]:
	return _down_the_edge(EDGE, count, CLEAR, SEED)


## [param count] trees down the edge at [param edge_x], between the ends of this
## made-up lawn at ±[constant LIMIT].
##
## [GrovePlan] used to write these two ends for its callers and no longer does —
## the modeled driveway's rows do not all run the same way, so the endpoints are
## the caller's to state. Every test below was written against that helper and
## reads better for keeping it, so it lives here now, where the made-up lawn it
## assumes is also written down.
func _down_the_edge(
	edge_x: float, count: int, keep_clear: float, plan_seed: int
) -> Array[Transform3D]:
	return GrovePlan.along(
		Vector3(edge_x, 0.0, -LIMIT), Vector3(edge_x, 0.0, LIMIT), count, keep_clear, plan_seed
	)
...
func test_a_run_clear_of_the_gap_is_planted_end_to_end() -> void:
	...
	var row: Array[Transform3D] = _down_the_edge(9.0, 4, CLEAR, SEED)
...
func test_a_left_hand_edge_is_planted_down_the_left() -> void:
	...
	for stand: Transform3D in _down_the_edge(-EDGE, 4, CLEAR, SEED):
...
func test_two_seeds_are_two_rows() -> void:
	...
	var other: Array[Transform3D] = _down_the_edge(EDGE, 5, CLEAR, SEED + 1)
...
func test_no_room_plants_nothing_rather_than_guessing() -> void:
	...
	assert_eq(_down_the_edge(EDGE, 4, 40.0, SEED).size(), 0)


func test_nothing_asked_for_is_nothing_planted() -> void:
	assert_eq(_down_the_edge(EDGE, 0, CLEAR, SEED).size(), 0)
	assert_eq(_down_the_edge(EDGE, -3, CLEAR, SEED).size(), 0)
